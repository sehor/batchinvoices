...
export function parseInvoices(workbook,buyerMap={},goodMap={}) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (row.buyerName) {
      //save pre invoice
      if (invoice) {
        invoices.push(Object.assign({}, invoice));
      }
      //start a new invoice
      invoice = {
        items: []
  ...
  invoices.forEach((invoice) => {
    ...
    if(!buyer.buyerTaxNum){
      buyer.buyerTaxNum = buyerMap[buyer.buyerTaxNum]||"not found"
    }
    if(!buyer.buyerBankInfo){
      buyer.buyerBankInfo = buyerMap[buyer.buyerBankInfo]||"not found"
    }
    if(!buyer.buyerAddrAndPhone){
      buyer.buyerAddrAndPhone = buyerMap[buyer.buyerAddrAndPhone]||"not found"
    }
    invoice.items.forEach((item)=>{
      if(!item.itemTaxNum){
        item.itemTaxNum = goodMap[item.itemTaxNum]||"not found"
      }
      if(!item.modelAndType){
        item.modelAndType = goodMap[item.modelAndType]||"not found"
      }
    })
    ...
    for (let i = 0; i < invoice.items.length; i++) {
      ...
      while (item.quantity > 0) {
        if (remain >= item.quantity * item.price) {
          ...
          //console.log(newInvoice)
          newInvoice = Object.assign({}, invoice, {
            items: []
          })
          remain = maxAmount
        }
...
function invioceXmlString(invoice,reviewer="",payee="") {
  let buyer = invoice.buyer;
  let fpHeader = `
...
export function invioceListXmlString(invoiceList,reviewer="",payee="") {
  let str = "";
  str = `
  ...
  invoiceList.forEach((invoice, index) => {
    str += `
        <Fp>
          <Djh>${index+1}</Djh >
          `
    str += invioceXmlString(invoice,reviewer,payee)
    str += `
        </FP>
        `
  })
...
// return {buyers,goods}
export async function buyersAndGoodsInfo(issueInvoicesExcel) {

  let h2c_buyer = new Map([
    ['buyerName', 3],
    ['buyerTaxNum', 4],
    ['buyerBankInfo', 5],
    ['buyerAddrAndPhone', 6],
  ])
  let h2c_good = new Map([
    ['itemName', 10],
    ['modelAndType', 11],
    ['unitType', 12],
    ['itemTaxNum', 18],
  ...
  //function  check dose a string contains any string in ignores
  ...
  let workBook = await readFile(issueInvoicesExcel)
  let sheet = workBook.getWorksheet(1)
  let buyers = new Map()
  let goods = new Map()
  let buyerCol = h2c_buyer.get('buyerName');
  sheet.eachRow({
    includeEmpty: false
  }, function (row, rowNumber) {
    let buyerName = row.getCell(buyerCol).value
    if (rowNumber > 6 &&
      !isIgnores(buyerName) &&
      buyerName) {
      let buyer = {}
      for (let [key, value] of h2c_buyer) {
        buyer[key] = row.getCell(value).value
      }
      buyers.set(buyerName, buyer)

      let goodName = row.getCell(h2c_good.get('itemName')).value.split('*').slice(-1)[0]
      let good = {}
      for (let [key, value] of h2c_good) {
        good[key] = row.getCell(value).value
      }
      good['itemName'] = goodName
      goods.set(goodName, good)
